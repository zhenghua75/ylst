...
class UsersController extends BaseController {
    ...
    public function checklogin() {
        ...
        if(!$verify->check(I('post.verifycode2'),'loginverify')){
            $this->error('验证码错误', U('Index/login'));
        }
        $db = D('Users');
        $uname = I('post.username');
        $viptime = $db->where(array('username' => $uname))->getfield('viptime');
        $where['username'] = I('post.username',false,'trim');

        $pwd = I('post.password',false,'trim,md5');
        $res = $db->where($where)->find();
        if (!$res && strlen($where['username']) == 11) {

            $res = $db->where(array('mp' => I('post.username','','trim')))->find();
        }
