...
class IndexController extends BaseController{
	protected function _initialize(){
		...
		if(S('zdydh') == ''){//自定义导航
			$db=M('Customs');
			$znav=$db->where(array('agentid'=>$agentid))->select();
			if($znav){
				S('zdydh',$znav);
			}
		}
	...
	public function index(){
	 ...
	 //      	}
		...
		if(session('displayid')){
			...
			$this->display('Index:'.ACTION_NAME.$displayid);
		}else{
	...
	public function fc(){
		...
		if(C('server_topdomain')=='pigcms.cn'){
			...
			if($displayid==2){
				...
				$id=I('get.id','intval');//$this->_get('id','intval');
				if (isset($id)){
	...
	public function about(){
		...
		$id=I('get.iid','intval');//$this->_get('iid','intval');
		$db=D('Img');
	...
	public function help(){
		if (isset($_GET['token'])){
			if (isset($_SESSION['uid'])){
				$thisWx=\Common\Api\apiInfo::info($_SESSION['uid'],0,I('get.token'));
	...
	public function login(){
		...
		$business = C('business');//include('./PigCms/Lib/ORG/Business.php');
		$i=0;
