...
class SceneController extends UserController{
	...
	public function _initialize()
	{
		...
		$scene_info = M("wechat_scene")->where(array("token" => $this->token, "is_open" => "1", "id" => I("get.sceneid", "intval")))->find();
		...
		if ($scene_info) {
			...
			$info["qrcode"] = ($scene_info["qrcode"] ? $info["qrcode"] : $scene_info["qrcode"]);
			$info["background"] = $scene_info["background"];
		}
		else {
			if ((ACTION_NAME == "wall") || (ACTION_NAME == "wall_pic")) {
				$info = M("Wall")->where(array("token" => $this->token, "id" => I("get.id", "intval")))->find();
				$info["open_wall"] = 1;
				$info["cheer"] = json_encode(explode(",", $info["cheer"]));
			}
			else if (ACTION_NAME == "shake") {
				$info = M("Shake")->where(array("token" => $this->token, "id" => I("get.id", "intval")))->find();
				$info["cheer"] = json_encode(explode("|", $info["cheer"]));
				$info["open_shake"] = 1;
			}
		}
	...
	public function set()
	{
		...
		$scene_info = $this->scene_db->where(array("token" => $this->token, "id" => I("get.id", "intval")))->find();
			...
			if ($this->scene_db->create($_POST)) {
				if ($scene_info) {
					...
					if ($this->scene_db->where(array("token" => $this->token, "id" => $id))->save($_POST)) {
						if (I("post.wall_id", "intval")) {
							$this->scene_db->where(array(
	"token" => $this->token,
	"id"    => array("neq", $id)
	))->save(array("is_open" => "0"));
							M("Wall")->where(array("token" => $this->token))->save(array("isopen" => "0"));
							M("Wall")->where(array("token" => $this->token, "id" => I("post.wall_id", "intval")))->save(array("isopen" => "1"));
						}
					}
	...
	{
		$result = M("Shake")->where(array("token" => $this->token, "id" => I("get.id", "intval")))->save(array("isact" => "1", "endtime" => time()));
		...
		if ($result) {
			...
			M("Shake")->where(array("token" => $this->token, "id" => I("get.id", "intval")))->save(array("isact" => "0", "endtime" => ""));
		}
	...
	{
		$result = M("Shake_rt")->where(array("token" => I("get.token"), "shakeid" => intval(I("get.id", "intval"))))->limit(0, 80)->order("count desc")->select();
	...
	public function percent($p, $t, $offset)
	{
		if ($t == 0) {
			$val = 1;
		}
	...
	{
		$prize = M("Wall_prize")->where(array("token" => $this->token, "sceneid" => I("get.sceneid", "intval")))->order("sort desc,id asc")->select();
		$users = M("Wall_member")->where(array("token" => $this->token, "act_type" => "3", "act_id" => I("get.sceneid", "intval")))->count();
	...
	public function prize_set()
	{
		...
		if (IS_POST) {
			if ($prize_db->create()) {
				if ($prize_info) {
					$_POST["token"] = $this->token;
					$prize_db->where(array("token" => $this->token, "id" => I("post.id", "intval"), "sceneid" => $sceneid))->save($_POST);
	...
	public function prizeRecords()
	{
		...
		$where["prize"] = I("get.pid", "intval");
		$where["sceneid"] = I("get.sceneid", "intval");
	...
	public function get_vote()
	{
		...
		$scene_id = I("get.scene_id", "intval");
	...
	public function _getMember($field, $id, $limit, $return, $ext)
	{
		$member_db = M("Wall_member");
