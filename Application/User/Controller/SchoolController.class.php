...
class SchoolController extends UserController{
    ...
    public function index(){
        ...
        $type = trim(filter_var(I('get.type'),FILTER_SANITIZE_STRING));
    ...
    public function del_item(){
        ...
        $type = trim(filter_var(I('get.deltype'),FILTER_SANITIZE_STRING));
        if(!in_array($type,$arrType)){
            exit($this->error('参数非法',U('Function/index',array('token'=>session('token')))));
        }
         if(IS_POST){
            $sid    = trim(filter_var(I('post.sid'),FILTER_VALIDATE_INT));
            //$type   = trim(filter_var(I('get.deltype'),FILTER_SANITIZE_STRING));
         ...
         $id     =   trim(filter_var(I('get.id'),FILTER_VALIDATE_INT));
         //$type   = filter_var(I('get.deltype'),FILTER_SANITIZE_STRING);
          if($type == 'students'){
    ...
    public function student_score(){
        ...
        $type   =   trim(filter_var(I('get.type'),FILTER_SANITIZE_STRING));
        $scid   =   intval(trim(filter_var(I('get.scid'),FILTER_VALIDATE_INT)));
        if('edit' == $type && '' != $scid){
        ...
        $_sid       = intval(trim(filter_var(I('get.id'),FILTER_VALIDATE_INT)));
        ...
        if(IS_POST){
            ...
            $_id     = intval(filter_var(I('post.id'),FILTER_VALIDATE_INT));
            $istoken = trim(filter_var(I('post.istoken'),FILTER_SANITIZE_STRING));
    ...
    public function subscribe_add(){
        $id     =   trim(filter_var(I('get.id'),FILTER_VALIDATE_INT));
        $type   =   trim(filter_var(I('get.type'),FILTER_SANITIZE_STRING));
        $addtype=   trim(filter_var(I('get.addtype'),FILTER_SANITIZE_STRING));
        ...
        if(IS_POST){
            $_POST['token']=session('token');
            $id     =   trim(filter_var(I('post.id'),FILTER_VALIDATE_INT));
            $type   =   trim(filter_var(I('post.type'),FILTER_SANITIZE_STRING));
            $addtype=   trim(filter_var(I('post.addtype'),FILTER_SANITIZE_STRING));
    ...
    public function res_manage(){
        ...
        $rid     =   trim(filter_var(I('get.id'),FILTER_VALIDATE_INT));
        $addtype=   trim(filter_var(I('get.addtype'),FILTER_SANITIZE_STRING));
        $this->assign('addtype',$addtype);
        $this->assign('rid',$rid);
    ...
    public function reservation_uinfo(){
        ...
        $token = trim(filter_var(I('get.token'),FILTER_SANITIZE_STRING));
    ...
    public function manage_del(){
        $id     = filter_var(I('get.id'),FILTER_VALIDATE_INT);
        $rid    = filter_var(I('get.rid'),FILTER_VALIDATE_INT);
        ...
        if(IS_POST){
            ...
            if(!$check){
                ...
                if(!$t_s_setindex->create()){
                    ...
                    $setid = filter_var(I('post.setid'),FILTER_VALIDATE_INT);
                    $type = filter_var(I('post.type'),FILTER_SANITIZE_STRING);
                    if('eidt'==$type && $setid != ''){

                   $o =  $t_s_setindex->where(array('setid'=>$setid, 'token'=>session('token')))->save($_POST);
                        if($o){

                            $this->handleKeyword($setid,'Schoolset',I('post.keyword'));
                        ...
                        if($id=$t_s_setindex->data($_POST)->add()){
                            ...
                            $this->handleKeyword($id,'Schoolset',I('post.keyword'));
    ...
    public function assess_add(){
        ...
        if(IS_POST){
            $_POST['token'] = session('token');
            $_tid =filter_var(I('post.tid'),FILTER_VALIDATE_INT);
            if(strtotime($_POST['birthdate']) >= strtotime("-10 Year")){
                exit($this->error('亲,才10来岁就当老师啦?'));
            }
            $info = filter_var(I('post.info'),FILTER_SANITIZE_STRING);
            if(empty($info)){
        ...
        $get_tid =filter_var(I('get.tid'),FILTER_VALIDATE_INT);
        $type = filter_var(I('get.type'),FILTER_SANITIZE_STRING);
    ...
    public function assess_course(){
        ...
        if(IS_POST){
            $_POST['token'] = session('token');
            $_id =filter_var(I('post.id'),FILTER_VALIDATE_INT);
    ...
    public function cookbook_add(){
        ...
        if(IS_POST){
            ...
            $_POST['begintime']  = strtotime(filter_var(I('post.begintime'),FILTER_SANITIZE_STRING));
            $_POST['endtime']    = strtotime(filter_var(I('post.endtime'),FILTER_SANITIZE_STRING));
            $_POST['type']       = filter_var(I('post.type'),FILTER_SANITIZE_STRING);
    ...
    public function cat_add(){
    	...
    	if (IS_POST) {
    		if(D('School_cat')->create()){
    			if($info){
    				M('School_cat')->where(array('token'=>$this->token,'id'=>I('post.id')))->save($_POST);
