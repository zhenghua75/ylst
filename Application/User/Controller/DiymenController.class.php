...
class DiymenController extends UserController{
	...
	public function class_del()
	{
		...
		if ($class == false) {
			...
			$class2 = M("Diymen_class")->where(array("token" => session("token"), "id" => I("get.id")))->order("sort desc")->find();

			if (0 != $class2["pid"]) {
				$back = M("Diymen_class")->where(array("token" => session("token"), "pid" => I("get.id")))->delete();
				$back2 = M("Diymen_class")->where(array("token" => session("token"), "id" => I("get.id")))->delete();
	...
	"query" => array("wecha_id" => "{wechat_id}")
			...
			$this->all_save("Diymen_class", "/index?id=" . I("get.id"));
		}
		else {
			$data = M("Diymen_class")->where(array("token" => session("token"), "id" => I("get.id")))->find();
			...
			$class2 = M("Diymen_class")->where(array("token" => session("token"), "pid" => I("get.id")))->order("sort desc")->find();

			foreach ($class as $key => $value ) {
				if (I("get.id") == $value["id"]) {
					unset($class[$key]);
				}
			}
			...
			$json["status"] = 200;
			$json["url"] = U("Diymen/class_edit", array("id" => I("get.id")));
