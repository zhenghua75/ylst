...
class ShakeController extends UserController{
	...
	public function show_rank(){
		$sceneid 	= I('get.sceneid',0,'intval');
		$wecha_id 	= M('Wall_member')->where(array('token'=>$this->token,'id'=>I('get.id','intval')))->getField('wecha_id');
		
		$where 		= array('token'=>$this->token,'shakeid'=>I('get.pid','intval'),'wecha_id'=>$wecha_id);
		...
		foreach ($list as $key => $value) {
			$rtwhere 	= array('token'=>$this->token,'shakeid'=>I('get.pid','intval'),'round'=>$value['round'],'is_scene'=>$where['is_scene']);
