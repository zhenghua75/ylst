...
class WeixinBillController extends UserController{
	...
	public function index()
	{
		...
		if (IS_POST) {
			$key = ($_POST["key"] ? htmlspecialchars($_POST["key"]) : "orderid");
			$searchkey = ($_POST["searchkey"] ? htmlspecialchars($_POST["searchkey"]) : "");
		}
		...
		$total_bill = array();
		$total_list = $data_weixin_bill->field("sum(price) as money, plat_type")->where($where)->group("`plat_type`")->select();

		foreach ($total_list as $tl ) {
			$tl["money"] = round($tl["money"] * 100) / 100;
