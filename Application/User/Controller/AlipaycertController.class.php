...
class AlipaycertController extends UserController{
	public $token;
	public function __initialize(){
		parent::_initialize();
		$this->canUseFunction('Alipaycert');
	...
	public function addCert($type = 'apiclient_cert',$url = ''){
		...
		if(empty($cert)){
			...
			if($insert){
				$this->success('上传成功',U('Alipaycert/index',array('token' => $this->token)));exit;
			}else{
			...
			if($update){
				$this->success('更新成功',U('Alipaycert/index',array('token' => $this->token)));exit;
			}else{
