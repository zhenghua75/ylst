...
class ProblemController extends UserController{
		...
		if(IS_POST){
			...
			if($set_db->create()){
				...
				if(empty($problem_info)){
					...
					$id = $set_db->add($_POST);
					if($id){
	                	$this->handleKeyword($id,'Problem',I('post.keyword'));
					}
				...
				//修改
					...
					if($offset){
						$this->handleKeyword(I('post.id','intval'),'Problem',I('post.keyword'));
					}
	...
	public function question(){
		$problem_id 	= I('get.problem_id',0,'intval');
