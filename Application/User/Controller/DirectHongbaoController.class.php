...
class DirectHongbaoController extends UserController{
	...
	public function hongbao_add()
	{
		if (IS_POST) {
			...
			else if (30 < strlen($data["send_name"])) {
				$this->error("红包发送者名称不超过30个字母");
			}

			if ($data["act_name"] == "") {
				$this->error("活动名称不能为空");
			}
			else if (30 < strlen($data["act_name"])) {
				$this->error("活动名称不超过30个字母");
			}

			if ($data["wishing"] == "") {
				$this->error("红包祝福语不能为空");
			}
			else if (120 < strlen($data["wishing"])) {
				$this->error("红包祝福语不超过120个字母");
			}
			...
			$this->assign("token", $this->token);
			$this->assign("send_type", (int) $_GET["send_type"]);
	...
	public function receive_recorde()
	{
		...
		$search_word = I("request.search_word", trim);
	...
	public function record_details()
	{
		...
		$hid = I('get.hid',0,'intval');
		$token = I("get.token");
		$page = ($_GET["page"] ? I("get.page", "intval") : 1);
		$type = I("get.type", "intval");
		$directhongbao_record = M("directhongbao_record")->where(array("id" => $id))->find();
		if (($directhongbao_record["mch_billno"] == "") || $directhongbao_record) {
			$this->error("操作失败");
		}
		...
		if (($record_details["status"] == "SUCCESS") && ($record_details["msg"]["hb_type"] == "GROUP")) {
			if (!$record_details["msg"]["hblist"]["hbinfo"]) {
				if (!$record_details["msg"]["hblist"]["hbinfo"][0]) {
	...
	public function del()
	{
		...
		if ($directhongbao) {
			...
			M("subscribe_reply")->where(array("reply_type" => 4, "relevance_id" => $id))->save(array("status" => 1));
	...
	private function request($fansinfo, $params)
	{
		...
		if ($res["status"] == "SUCCESS") {
			...
			$record["fans_nickname"] = ($fansinfo["nickname"] ? trim($fansinfo["nickname"]) : "匿名用户");
