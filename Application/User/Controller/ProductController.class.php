...
class ProductController extends UserController{
	...
	public function catAdd(){
		if(IS_POST){
			if ($this->isDining){
				$this->insert('Product_cat','/cats?dining=1&parentid='.I('post.parentid'));
			}else {
				$this->insert('Product_cat','/cats?parentid='.I('post.parentid'));
			}
		}else{
			$parentid=intval($_GET['parentid']);
			$parentid=$parentid==''?0:$parentid;
		...
		if(IS_POST){ 
			...
			if($data->create()){
				if($data->where($where)->save($_POST)){
					if (!$this->isDining){
						$this->success('修改成功',U('Product/cats',array('token'=>session('token'),'parentid'=>I('post.parentid'))));
					}else {
						$this->success('修改成功',U('Product/cats',array('token'=>session('token'),'parentid'=>I('post.parentid'),'dining'=>1)));
					}
