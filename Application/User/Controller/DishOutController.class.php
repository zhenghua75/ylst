...
class DishOutController extends UserController{
	public $_cid = 0;
	...
	public function manageTime()
	{
		...
		if (IS_POST) {
			$zc_sdate = I("post.zc_sdate");
			$zc_edate = I("post.zc_edate");
			$wc_sdate = I("post.wc_sdate");
			$wc_edate = I("post.wc_edate");
			$permin = intval(I("post.permin"));
			$removing = intval(I("post.removing"));
			$area = htmlspecialchars(I("post.area"), ENT_QUOTES);
			$sendtime = intval(I("post.sendtime"));
			$simage = I("post.simage");
			$tourl = I("post.tourl");
			$simage = array("img" => $simage, "tourl" => $tourl);
			$tid = intval(I("post.tid"));
			$data["zc_sdate"] = (!$zc_sdate ? strtotime(date("Y-m-d ") . $zc_sdate) : 0);
			$data["zc_edate"] = (!$zc_edate ? strtotime(date("Y-m-d ") . $zc_edate) : 0);
			$data["wc_sdate"] = (!$wc_sdate ? strtotime(date("Y-m-d ") . $wc_sdate) : 0);
			$data["wc_edate"] = (!$wc_edate ? strtotime(date("Y-m-d ") . $wc_edate) : 0);
	...
	public function basePrice()
	{
		...
		if (IS_POST) {
			$stype = intval(I("post.stype"));
			$pricing = intval(I("post.pricing"));
			$tid = intval(I("post.tid"));
	...
	public function myReply()
	{
		...
		if (IS_POST) {
			$data["keyword"] = htmlspecialchars(I("post.keyword"), ENT_QUOTES);
			$data["rtitle"] = htmlspecialchars(I("post.rtitle"), ENT_QUOTES);
			$data["rinfo"] = htmlspecialchars(I("post.rinfo"), ENT_QUOTES);
			$data["picurl"] = htmlspecialchars(I("post.picurl"), ENT_QUOTES);
			$tid = intval(I("post.tid"));
	...
	{
		$status = (I("get.status") ? intval(I("get.status")) : 0);
		$t = (I("get.t") ? intval(I("get.t")) : 0);
		...
		$Page = new \Think\Page($count, 20);
		$show = $Page->show();
		$orders = (!$where ? $dish_order->where($where)->order("id DESC")->limit($Page->firstRow . "," . $Page->listRows)->select() : false);
	...
	{
		$id = (I("post.oid") ? intval(I("post.oid")) : 0);
		$dishOrder = M("Dish_order");
