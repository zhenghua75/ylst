...
class ImgBaseController extends UserController{ 
	...
	private function _getGroup() {
		...
		$result = \Extend\HttpClient::getInstance()->get($url);
	...
	public function insert(){
		...
		if($_POST['pc_cat_id'] && $_POST['pc_show']){
			...
			$data_pc_news['title'] = I('post.title');
			$data_pc_news['info'] = I('post.text');
			$data_pc_news['pic'] = I('post.pic');
			$data_pc_news['content'] = I('post.info','stripslashes,htmlspecialchars_decode');
	...
	public function syncNews($type) {
		...
		if (isset($_POST['pic'])) {
			...
			if (empty($thumb_media_id)) {
				if(strpos($_POST['pic'],C('site_url')) !== false){
					...
					$imgStr=\Extend\HttpClient::getInstance()->get($_POST['pic']);
					if (!$imgStr){
		...
		$params = array();
		$content = str_replace('&amp;', '&', I('post.info'));
		...
		$params['post']['articles'] = array(
				array(
						...
						'author' => I('post.writer'),
						'digest' => I('post.text'),
						'show_cover_pic' => I('post.showpic'),
						'content' => $content,
						'content_source_url' => $this->convertLink(I('post.url')),
				)
		);
			$data = M('Img')->where(array('id'=>I('post.id')))->find();
			if ($data['media_id']) {
