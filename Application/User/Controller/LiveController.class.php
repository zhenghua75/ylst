...
class LiveController extends UserController{
	...
	public function add()
	{
		...
		if (IS_POST) {
			if (D("Live")->create()) {
				if ($live_info) {
					$up_where = array("token" => $this->token, "id" => I("post.id", "intval"));
					M("live")->where($where)->save($_POST);
					$this->handleKeyword(I("post.id", "intval"), "Live", I("post.keyword"));
					...
					$_POST["add_time"] = time();
					$id = M("live")->add($_POST);
					$this->handleKeyword($id, "Live", I("post.keyword"));
	...
	public function content_add()
	{
		...
		if (IS_POST) {
			if ($info) {
				$where = array("token" => $this->token, "live_id" => I("post.live_id", "intval"), "id" => I("post.id", "intval"));
				M("Live_content")->where($where)->save($_POST);
				$this->success("修改成功", U("Live/content", array("token" => $this->token, "id" => I("post.live_id", "intval"))));
			}
			else {
				$_POST["add_time"] = time();
				$_POST["token"] = $this->token;
				...
				if (M("Live_content")->add($_POST)) {
					$this->success("添加成功", U("Live/content", array("token" => $this->token, "id" => I("post.live_id", "intval"))));
				}
			}
		}
		else {
		...
		if (IS_POST) {
			if (D("Live_company")->create()) {
				if ($info) {
					...
					$this->success("修改成功", U("Live/company", array("token" => $this->token, "id" => I("post.live_id", "intval"))));
				}
				else {
					$_POST["token"] = $this->token;

					if (M("Live_company")->add($_POST)) {
						$this->success("添加成功", U("Live/company", array("token" => $this->token, "id" => I("post.live_id", "intval"))));
					}
				}
			}
			else {
