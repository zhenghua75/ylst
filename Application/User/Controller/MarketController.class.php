...
class MarketController extends UserController{
	public $market_db;
	public $area_db;
	public $park_db;
	public $cate_db;
	...
	public function _initialize()
	{
		...
		if ($market_id && (ACTION_NAME != "index")) {
	...
	public function index()
	{
		...
		if (IS_POST) {
			$slide["slide_title"] = (I("request.slid_title") ? I("request.slid_title") : "");
			$slide["slide_link"] = I("request.link");
			$slide["slide_url"] = I("request.slide");

			if ($this->market_db->create()) {
				if ($market) {
					$this->market_db->where($where)->save($_POST);
					$this->_market_slide(I("post.market_id", "intval"), $slide, "update");
					$keyword["pid"] = I("post.market_id", "intval");
					$keyword["module"] = "Market";
					$keyword["token"] = $this->token;
					$keyword["keyword"] = I("post.keyword");
					$keyword_db->where(array("token" => $this->token, "pid" => I("post.market_id", "intval"), "module" => "Market"))->save($keyword);
			...
			$this->assign("default", "__PUBLIC__/static/market/default.jpg");
	...
	public function _create_nav($id)
	{
		...
		$pic = array("__PUBLIC__/static/attachment/icon/white/1.png", "__PUBLIC__/static/attachment/icon/white/4.png", "__PUBLIC__/static/attachment/icon/white/9.png", "__PUBLIC__/static/attachment/icon/white/7.png", "__PUBLIC__/static/attachment/icon/white/14.png", "__PUBLIC__/static/attachment/icon/white/15.png");
	...
	public function _market_slide($market_id, $data, $handle)
	{
	...
	{
		$where = array("token" => $this->token, "market_id" => $this->market_id, "area_id" => I("get.area_id", "intval"));
		...
		if (IS_POST) {
			if ($this->area_db->create()) {
				if ($area) {
					$_POST["add_time"] = time();
					$_POST["is_use"] = ($_POST["is_use"] ? "0" : $_POST["is_use"]);
					$_POST["area_intro"] = I("post.area_intro");
	...
	{
		$where = array("token" => $this->token, "area_id" => I("get.area_id", "intval"));
	...
	{
		$where = array("token" => $this->token, "market_id" => $this->market_id, "park_id" => I("get.park_id", "intval"));
		...
		if (IS_POST) {
			if ($this->park_db->create()) {
				if ($park) {
					$_POST["add_time"] = time();
					$_POST["is_use"] = ($_POST["is_use"] ? "0" : $_POST["is_use"]);
					$_POST["park_intro"] = I("post.park_intro");
	...
	{
		$where = array("token" => $this->token, "park_id" => I("get.park_id", "intval"));
	...
	{
		$where = array("token" => $this->token, "market_id" => $this->market_id, "cate_id" => I("get.cate_id", "intval"));
		$cate = $this->cate_db->where($where)->find();

		if (IS_POST) {
			if ($this->cate_db->create()) {
				if ($cate) {
					$_POST["is_show"] = ($_POST["is_show"] ? "0" : $_POST["is_show"]);
	...
	{
		$where = array("token" => $this->token, "cate_id" => I("get.cate_id", "intval"));
	...
	public function tenant_set()
	{
		...
		$this->assign("area_list", $area_list);
		$this->assign("cate_list", $cate_list);
		$tenant_info = $this->tenant_db->where(array("token" => $this->token, "market_id" => $this->market_id, "id" => I("get.id", "intval")))->find();

		if (IS_POST) {
			if ($this->tenant_db->create()) {
				if ($tenant_info) {
					$this->tenant_db->where(array("token" => $this->token, "market_id" => $this->market_id, "isbranch" => "1", "id" => I("get.id", "intval")))->save($_POST);
	...
	public function wap_nav_set()
	{
		...
		if (IS_POST) {
			if ($nav_db->create()) {
				if ($nav_info) {
					$_POST["is_show"] = ($_POST["is_show"] ? "0" : $_POST["is_show"]);
					$nav_db->where(array("token" => $this->token, "nav_id" => I("post.nav_id", "intval")))->save($_POST);
	...
	{
		$nav_db = M("Market_nav");
		$where = array("token" => $this->token, "nav_id" => I("get.nav_id", "intval"));
