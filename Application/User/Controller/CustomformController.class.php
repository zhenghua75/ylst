...
class CustomformController extends UserController{
	public $token;
	public $set_id;
	public $field_db;
	...
	public function _initialize(){
		...
		$this->set_id 	= I('get.set_id',0,'intval');
	...
	public function set(){
		...
		if(IS_POST){
			if($field_info){//修改
				...
				$this->success('修改成功',U('Customform/index',array('token'=>$this->token,'set_id'=>$this->set_id)));
			}else{//添加
				...
				if($this->field_db->create($_POST)){
					...
					$this->success('添加成功',U('Customform/index',array('token'=>$this->token,'set_id'=>$this->set_id)));
				}else{
