...
class  WechatbehaviorController extends UserController{
	public $token;
	private $data;
	private $openid;
	...
	public function statisticsTrend(){
		$now=time();
		$this->modules=$this->_modules();
		$function = M("function")->field('name,funname')->select();
		foreach($function as $fvo){
			$funname[strtolower($fvo['funname'])] = $fvo['name'];
		}
		$where=array('token'=>$this->token);
		$days=7;
		$this->assign('days',$days);
