...
class StoreController extends UserController{
		...
		if (IS_POST) {
			...
			if ($data->create()) {
				if ($data->where($where)->save($_POST)) {
					$this->success('修改成功', U('Store/index', array('token' => session('token'), 'cid' => $this->_cid, 'parentid' => I('post.parentid'))));
				} else {
					$this->error('操作失败');
				}
			} else {
	...
	public function normsAdd() 
	{
		...
		if($data = M('Product_cat')->where(array('id' => I('get.catid'), 'token' => session('token'), 'cid' => $this->_cid))->find()){
		...
		if (IS_POST) { 
            ...
            $id = intval(I('post.id'));
            if ($id) {
	            $where = array('id' => $id, 'type' => $type, 'catid' => I('get.catid'));
				$check = $data->where($where)->find();
				if ($check == false) $this->error('非法操作');
            }
			if ($data->create()) {
				if ($id) {
					if ($data->where($where)->save($_POST)) {
						$this->success('修改成功', U('Store/norms',array('token' => session('token'), 'catid' => I('post.catid'), 'type' => $type)));
					} else {
						$this->error('操作失败');
					}
				} else {
					if ($data->add($_POST)) {
						$this->success('添加成功', U('Store/norms',array('token' => session('token'), 'catid' => I('post.catid'), 'type' => $type)));
					} else {
						$this->error('操作失败');
					}
				}
			...
			$data = M('Norms')->where(array('id' => I('get.id'), 'type' => $type, 'catid' => I('get.catid')))->find();
			//print_r($data);die;
			$this->assign('catid', I('get.catid'));
			$this->assign('type', $type);
	...
	{
		if ($checkdata = M('Product_cat')->where(array('id' => I('get.catid'), 'token' => session('token'), 'cid' => $this->_cid))->find()) {
			$this->assign('catData', $checkdata);
        } else {
        	$this->error("没有选择相应的分类.", U('Store/index'));
        }
		if (IS_POST) { 
			...
			if ($data->create()) {
				if ($id) {
					if ($data->where($where)->save($_POST)) {
						$this->success('修改成功', U('Store/attributes',array('token' => session('token'), 'catid' => I('post.catid'))));
					} else {
						$this->error('操作失败');
					}
				} else {
					if ($data->add($_POST)) {
						$this->success('添加成功', U('Store/attributes',array('token' => session('token'), 'catid' => I('post.catid'))));
					} else {
						$this->error('操作失败');
					}
				}
			...
			$data = M('Attribute')->where(array('id' => I('get.id'), 'token' => session('token'), 'cid' => $this->_cid, 'catid' => I('get.catid')))->find();
			$this->assign('catid', I('get.catid'));
			$this->assign('token', session('token'));
			$this->assign('set', $data);
			$this->display();	
...
//	        }
	...
	public function productSave() {
		...
		if($_POST['pc_cat_id'] && $_POST['pc_show'] && empty($_POST['pid'])){
			...
			$data_pc_product['title'] = I('post.name');
			$data_pc_product['content'] = I('post.intro','stripslashes,htmlspecialchars_decode');
