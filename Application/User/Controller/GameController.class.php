...
class GameController extends UserController{
	...
	public function config(){
		...
		if (IS_POST){
			...
			'wxlogo'=>I('post.wxlogo'),
			'link'=>I('post.link'),
			'attentionText'=>I('post.attentionText'),
			);
			if (!$config){
	...
	public function gameSet(){
		...
		if (IS_POST){
			...
			if ($wxtype['winxintype'] == 3) {
				$gameqr = $this->getQrcode(I('post.keyword'));
			}else{
				if (empty($wxtype['qr'])) {
			...
			$data=array(
				...
				'time'=>time(),
				'gameid'=>$thisGame['id'],
				'share_callback'=>I('post.share_callback'),
			...
			if ($selfs){
				$comma='';
				foreach ($selfs as $s){
					$selfValues['self_'.$s['id']]=I('post.self_'.$s['id']);
			...
			$gameSet['is_phone'] = I('post.is_phone');
			$gameSet['is_attention'] = I('post.is_attention');
			...
			$gameSet['keyword'] = I('post.keyword');
			$gameSet['share_callback'] = I('post.share_callback');
	...
	function record(){
		$where 	= array('token'=>$this->token,'gameid'=>I('get.id','intval'));
	...
	function record_del(){
		$where 	= array('token'=>$this->token,'id'=>I('get.id','intval'));
		...
		if(M('Game_records')->where($where)->delete()){
			$this->success('删除成功',U('Game/record',array('token'=>$this->token,'id'=>I('get.rid','intval'))));
		}	
	...
	function gamearr(){
		S('game_'.I('get.token').'_'.I('get.id'),$_POST);
	}
