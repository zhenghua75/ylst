...
class CrowdfundingController extends UserController{
	public $basis_db;
	public $reward_db;
	...
	public function _initialize(){
		...
		$checkFunc=new \Extend\checkFunc();if (!function_exists('fdsrejsie3qklwewerzdagf4ds')){exit('error-4');}
			...
			if($this->basis_db->create()){  
                if($info){
                	if($this->ck_status($info['id'])){
	                	$save_where 	= array('token'=>$this->token,'id'=>I('post.id',0,'intval'));
	                	$this->basis_db->where($save_where)->save($_POST);	           
	                	$this->handleKeyword(I('post.id','intval'),'Crowdfunding',I('post.keyword'));
	                    $this->success('修改成功');
                	}else{
                	...
                	if($id){
                		$this->ck_status($id,1);
                		$this->handleKeyword($id,'Crowdfunding',I('post.keyword'));
			...
			if($this->reward_db->create()){
                if($reward_info){
                	if($this->ck_status($pid)){
	                	$save 	= array('token'=>$this->token,'pid'=>I('post.pid','intval'),'id'=>I('post.id','intval'));
	                	$this->reward_db->where($save)->save($_POST);
	                	$this->redirect('Crowdfunding/reward_set', array('token'=>$this->token,'pid' => I('post.pid','intval')));
                	}else{
                		$this->success('项目已结束，修改失败',U('Crowdfunding/reward_set',array('token'=>$this->token,'pid' => I('post.pid','intval'))));
                	}
               	}else{
               		$rid 	= $this->reward_db->add($_POST);
               		if($rid && empty($reward_list)){
               			$this->ck_status(I('post.pid','intval'),2);
               		}
               		$this->redirect('Crowdfunding/reward_set', array('token'=>$this->token,'pid' => I('post.pid','intval')));
               	}
            }else{
	...
	public function affirm(){
		...
		if($_POST){
			if($status == 3){
				...
				if($this->basis_db->where(array('token'=>$this->token,'id'=>I('post.pid','intval'),'status'=>array('eq',2)))->save(array('start'=>time(),'status'=>3))){
