...
class AlipayController extends UserController{
		...
		if (($needFee < $moneyBalance) || ($needFee == $moneyBalance)) {
			...
			if ($this->isAgent) {
				...
				if ($wxusers) {
					...
					M("Agent_expenserecords")->add(array("agentid" => $this->thisAgent["id"], "amount" => -$price, "des" => $this->user["username"] . "(uid:" . $this->user["id"] . ")延期" . $month . "个月，共" . count($wxusers) . "个公众号", "status" => 1, "time" => time()));
				}
			}
	...
	{
		if ((I("post.price") == false) || (I("post.uname") == false)) {
			$this->error("价格和用户名必须填写");
		}

		$url = str_replace(".cn", ".com", C("site_url"));
		header("Location:" . $url . "/index.php?g=User&m=Alipay&a=post&price=" . I("post.price") . "&uname=" . I("post.uname") . "&uid=" . session("uid") . "&groupid=" . I("post.group") . "&num=" . I("post.num"));
	}
	...
	public function post()
	{
		...
		$exter_invoke_ip = "";
		$body = $subject;
		$data = M("Indent")->data(array("uid" => session("uid"), "month" => intval(I("post.num")), "title" => $subject, "uname" => I("post.uname"), "gid" => I("post.gid"), "create_time" => time(), "indent_id" => $out_trade_no, "price" => $total_fee))->add();
	...
	public function recharge()
	{
		...
		$exter_invoke_ip = "";
		$body = $subject;
		$data = M("Indent")->data(array("uid" => session("uid"), "month" => 0, "title" => $subject, "uname" => I("post.uname"), "gid" => 0, "create_time" => time(), "indent_id" => $out_trade_no, "price" => $total_fee))->add();
		$show_url = rtrim(C("site_url"), "/");
		$parameter = array("service" => "create_direct_pay_by_user", "partner" => trim(C("alipay_pid")), "payment_type" => $payment_type, "notify_url" => $notify_url, "return_url" => $return_url, "seller_email" => $seller_email, "out_trade_no" => $out_trade_no, "subject" => $subject, "total_fee" => $total_fee, "body" => $body, "show_url" => $show_url, "anti_phishing_key" => $anti_phishing_key, "exter_invoke_ip" => $exter_invoke_ip, "_input_charset" => trim(strtolower("utf-8")));
		$alipaySubmit = new AlipaySubmit($this->setconfig());
		$html_text = $alipaySubmit->buildRequestForm($parameter, "get", "确认");
	...
	{
		$Alipayconfig["partner"] = trim(C("alipay_pid"));
		$Alipayconfig["key"] = trim(C("alipay_key"));
		$Alipayconfig["sign_type"] = strtoupper("MD5");
		$Alipayconfig["input_charset"] = strtolower("utf-8");
		$Alipayconfig["cacert"] = getcwd() . "\PigCms\Lib\ORG\Alipay\cacert.pem";
