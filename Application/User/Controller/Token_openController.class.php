...
class Token_openController extends UserController{

	public function add(){

		if ($this->isAgent){
			$fun=M('Agent_function')->where(array('id'=>intval(I('get.id'))))->find();
		}else {
			$fun=M('Function')->where(array('id'=>intval(I('get.id'))))->find();
		}
