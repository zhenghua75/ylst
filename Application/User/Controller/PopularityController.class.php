...
class PopularityController extends UserController{
		...
		if(IS_POST){
			...
			if(D('Popularity')->create()){
				if($pop_info){
					...
					$this->prize_set(I('post.id',0,'intval'),$prize,'save');
					$this->handleKeyword(I('post.id',0,'intval'),'Popularity',I('post.keyword'));
					$this->success('修改成功',U('Popularity/index',array('token'=>$this->token)));
				}else{
					$id                 = D('Popularity')->add($_POST);
					if($id){
						$this->prize_set($id,$prize,'add');
						$this->handleKeyword($id,'Popularity',I('post.keyword'));
	...
	public function prize_log(){
		...
		if(IS_POST){
			$min 	= I('post.min',0,'intval');
			$max 	= I('post.max',0,'intval');
			...
			if(!empty($_GET['min'])){
				$min 	= I('get.min',0,'intval');
				$max 	= I('get.max',0,'intval');
				if($min && $max){
					$where['share_count'] 	= array('BETWEEN',array($min,$max));
				}
