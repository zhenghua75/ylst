...
class EstateController extends UserController{
	...
	public function set(){
        ...
        $es_data = $data->where(array('token'=>$this->token,'id'=>I('get.id','intval')))->find();
           ...
           if($es_data == null){
            ...
            $wh = array('token'=>session('token'),'id'=>I('post.id'));
             if($data->where($wh)->save($_POST)){
                    ...
                    $data1['module']='Estate';
                    $data1['token']=session('token');
                    $da['keyword']=trim(I('post.keyword'));
            ...
            if(empty($es_data)){//默认背景图
                for ($i=1; $i <=3 ; $i++) { 
                    if($es_data['slide'.$i] == ''){
                        $es_data['slide'.$i]    = $this->staticPath.'/tpl/static/attachment/background/view/'.$i.'.jpg';
                    }
                }
            }
        ...
        if(IS_POST){
            if($nav_db->create()){   
                if($nav_info){
                    ...
                    $nav_db->where(array('token'=>$this->token,'id'=>I('post.id','intval'),'pid'=>$pid))->save($_POST);
    ...
    public function nav_del(){
        $nav_db = M('Estate_nav');
        $pid   = I('get.pid',0,'intval');
        $where  = array('token'=>$this->token,'id'=>I('get.id','intval'),'pid'=>$pid);
    ...
    public function _create_nav($id){
       $name = array(
        ...
        $pic = array(
            './__PUBLIC__/web/images/plugmenu6.png',
            './__PUBLIC__/web/images/plugmenu4.png',
            './__PUBLIC__/web/images/plugmenu7.png',
            './__PUBLIC__/web/images/plugmenu17.png',
            './__PUBLIC__/web/images/plugmenu15.png',
            './__PUBLIC__/web/images/plugmenu8.png',    
            './__PUBLIC__/web/images/plugmenu19.png',
        );
    ...
    public function son_add(){
        ...
        if(IS_POST){
            ...
            if($check == null){
             ...
             $wh = array('token'=>session('token'),'id'=>I('post.id'),'pid'=>$pid);
    ...
    public function album_add(){
        ...
        if(IS_POST){
            if($check == NULL){
                $check_ex = $t_album->where(array('token'=>session('token'),'poid'=>I('post.poid'),'pid'=>$pid))->find();
                if($check_ex){
    ...
    public function aboutus(){
        ...
        if(IS_POST){
            ...
            if($check == null){
             ...
             $wh = array('id'=>I('post.id'),'token'=>session('token'));
