...
class CarController extends UserController{
    ...
    public function brand_edit(){
        if(IS_POST){
            $data=D('Car');
            $where=array('id'=>(int)I('get.bid'),'token'=>session('token'));
    ...
    public function addseries(){
         ...
         $this->assign('brands',$brands);
         if(IS_POST){
            $exp_brand = explode('@',I('post.brand'));
            $data=D('Carseries');
            $_POST['token'] = session('token');
    ...
    public function get_car_brand(){
        //品牌 和 车系
         ...
         $arr=$t_carseries->where($where)->field('id,brand_id,brand,name')->order('id asc')->select();
    ...
    public function edit_carmodel(){
        ...
        if(IS_POST){
            $data=D('Carmodel');
            $where=array('id'=>(int)I('get.id'),'token'=>session('token'));
            $check=$data->where($where)->find();
            if($check==false)$this->error('非法操作');
            if($data->create()){
                if($data->where($where)->save($_POST)){
    ...
    public function add_res(){
        $this->assign('addtype',I('get.addtype'));
    ...
    public function edit_salers(){
        if(IS_POST){
            ...
            if($data->create()){
                $where2 = array('id'=>(int)I('post.id'),'token'=>session('token'));
    ...
    public function owner_uinfo(){
        ...
        if(IS_POST){
             $_POST['car_insurance_lastDate'] = I('post.insurance_Date');
             $_POST['car_insurance_lastCost'] = I('post.insurance_Cost');
             $_POST['car_care_mileage'] = I('post.care_mileage');
             $_POST['car_care_lastDate'] = I('post.car_care_Date');
             $_POST['car_care_lastCost'] = I('post.car_care_Cost');
            $id = I('post.id');
            $token = session('token');
            $where =  array('id'=>$id,'token'=>$token);
    ...
    public function cat_add(){
        ...
        if (IS_POST) {
            if(D('Carcat')->create()){
                if($info){
                    M('Carcat')->where(array('token'=>$this->token,'id'=>I('post.id')))->save($_POST);
