...
class RepastStaffController extends BaseController{
	public $_cid = 0;
	public $token = '';
	public function _initialize() {
		parent::_initialize();
        $this->_cid = I('get.cid',session('ycompany_id'), 'intval');
        $this->token = I('get.token',session('RepastStaff_token'));
    ...
    public function mtlogin() {
        if (IS_POST) {
            ...
            $username = htmlspecialchars(I('post.username'), ENT_QUOTES);
            $pwd = I('post.password');
    ...
    public function toSwitchStatus() {
        $typ = I('post.typ',0,'intval');
        $tid = I('post.tid',0,'intval');
        $vv =  I('post.vv',0,'intval');
        $dtid = ('post.dtid',0,'intval');
        $Diningdb = M('Dining_table');
        switch ($typ) {
            case 1:
                if ($vv == 1) {
