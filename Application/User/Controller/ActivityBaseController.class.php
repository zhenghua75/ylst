...
class ActivityBaseController extends UserController{
		...
		if(IS_POST){
			...
			if($_POST['enddate'] < $_POST['statdate']){
				...
				if($data->create()!=false){
					if($id=$data->add()){
						$data2['zjpic']=$id;
						$data2['keyword']=I('post.keyword');
						$data2['token']=$this->token;
						$data2['parssword']=I('post.parssword');
						M('Lottery')->add($data2);
						$this->handleKeyword($id,'Activity',I('post.keyword'));
			...
			if($_POST['enddate'] < $_POST['statdate']){
				...
				if($check==false)$this->error('非法操作');
					if($data->where($where)->save($_POST)){
						...
						$this->handleKeyword($lid,'Activity',I('post.keyword'));
