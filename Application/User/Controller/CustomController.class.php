...
class CustomController extends UserController{
	public $token;
	public $set_db;
	public $limit_db;
	public $field_db;
	...
	public function index(){
		...
		if(I('post.search')){
			$where['title|keyword']	= array('like','%'.I('post.search').'%');
		}
	...
	public function set(){
		...
		$where 		= array('token'=>$this->token,'set_id'=>I('get.set_id',0,'intval'));
		...
		if(IS_POST){
			...
			$_POST['succ_info'] = empty($_POST['succ_info'])?'提交成功':I('post.succ_info');
			$_POST['err_info']	= empty($_POST['succ_info'])?'提交失败':I('post.err_info');	

			//限制表数据
			if($_POST['endtime']){
				$limit['enddate']		=	strtotime(I('post.end_value').' 23:59:59');		
			}else{
				$limit['enddate']		= '';
			}

			if($_POST['today_total']){
				$limit['today_total']	=	I('post.today_value',0,'intval');		
			}else{
				$limit['today_total']	= 0;
			}
			if($_POST['sub_total']){
				$limit['sub_total']		=	I('post.sub_value',0,'intval');
			}else{
				$limit['sub_total']		= 0;
			}
			...
			if($set_info){
				if($this->set_db->create()){
					$_POST['detail']	= I('post.detail');
					$this->set_db->where($where)->save($_POST);//更新设置表
					$this->limit_db->where(array('limit_id'=>$set_info['limit_id']))->save($limit);//更新限制表
                	$this->handleKeyword(I('post.set_id','intval'),'Custom',I('post.keyword'));	
                	$this->success('修改成功',U('Custom/set',array('token'=>$this->token,'set_id'=>I('get.set_id','intval'))));
				}else{
				...
				if($this->set_db->create()){
                	...
                	$this->handleKeyword($id,'Custom',I('post.keyword'));
	...
	public function info(){
		...
		if($name){
			$thisOrder = M('custom_order')->where(array('token'=>$this->token,'set_id'=>$set_id,'orderid'=>$name))->find();
			if($thisOrder){
				$where['info_id'] = $thisOrder['state'];
			}else{
		...
		foreach($list as $key=>$value){
			...
			$list[$key]['order'] = M('custom_order')->where(array('token'=>$this->token,'wecha_id'=>$value['wecha_id'],'set_id'=>$value['set_id'],'paid'=>1,'state'=>$value['info_id']))->find();
		}

		$field = $this->field_db->where(array('token'=>$this->token,'set_id'=>$set_id))->order('sort desc')->limit('6')->field('field_name')->select();
		
		$this->assign('thisForm',M('custom_set')->where(array('token'=>$this->token,'set_id'=>$set_id))->find());
	...
	public function detail(){
		$where 		= array('token'=>$this->token,'info_id'=>I('get.info_id',0,'intval'));
...
/*--------------------------------------------表单管理项-------------------------------------------*/
	...
	public function forms_del(){
		$where = array('token'=>$this->token,'field_id'=>I('get.field_id','intval'));
		$info  = $this->info_db->where(array('token'=>$this->token,'set_id'=>I('get.set_id','intval')))->getField('sub_info');
		if ($info) {
