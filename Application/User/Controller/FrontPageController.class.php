...
class FrontPageController extends UserController{
	...
	public function addaction()
	{
		if (IS_POST) {
			...
			$data["remind_img"] = I("post.remind_img");
			$data["remind_link"] = I("post.remind_link");
			$data["total_create"] = I("post.total_create", "intval");
			$data["day_create"] = I("post.day_create", "intval");
			...
			$data["sponsors"] = I("post.sponsors");
			$data["start_time"] = strtotime($_POST["start_time"]);
			$data["end_time"] = strtotime($_POST["end_time"]);
			$data["latest_count"] = ($_POST["latest_count"] ? I("post.latest_count", "intval") : 10);
	...
	public function addconfigure()
	{
		if (IS_POST) {
			...
			if ($exists) {
				...
				$congfig["token"] = $this->token;
				$congfig["addtime"] = $exists["addtime"];
				$addconfigure = M("frontpage_configure")->add($congfig);
	...
	public function fansnews()
	{
		...
		$Page = new \Think\Page($newstotal, 15);
	...
	public function definenews()
	{
		...
		$newstotal = M("frontpage_makenews")->where($where)->count();
		$Page = new \Think\Page($newstotal, 20);
		$findnews = M("frontpage_makenews")->where($where)->limit($Page->firstRow . "," . $Page->listRows)->order("id desc")->select();
		$this->assign("list", $findnews);
		$this->assign("page", $Page->show());
		$this->assign("search_word", $search_word);
	...
	public function adddefinenews()
	{
		if (IS_POST) {
			...
			$data["news_txt"] = I("post.news_txt");
			$data["spd"] = 7;
			$data["per"] = I("post.per", "intval");
			$data["status"] = I("post.status", "intval");
			$id = I("post.id", "intval");
	...
	private function getAccessToken($post)
	{
		...
		$response = \Extend\HttpClient::getInstance()->setOption(CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_0)->post($url, $params);
	...
	public function IsValid($access_token)
	{
		...
		$params["post"]["per"] = 1;
		$params["post"]["spd"] = 5;
		$binary_file = \Extend\HttpClient::getInstance()->post($url, $params);
