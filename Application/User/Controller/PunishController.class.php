...
class PunishController extends UserController{
		...
		if(IS_POST){
			if(D('Punish')->create()){
				...
				if(empty($info)){
					...
					$where 	= array('token'=>$this->token,'id'=>I('post.id','intval'));
					$offset = M('Punish')->where($where)->save($_POST);//更新设置表
					if($offset){
						$keyword['pid']		= I('post.id','intval');
						$keyword['module']	= 'Punish';
						$keyword['token']	= $this->token;
						$keyword['keyword']	= I('post.keyword');
						$keyword_db->where(array('token'=>$this->token,'pid'=>I('post.id','intval'),'module'=>'Problem'))->save($keyword);
					}
