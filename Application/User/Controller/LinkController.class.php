...
class LinkController extends UserController{
	public $where;
	public $modules;
	public $arr;
	...
	private function _filterModule($modules) {
		...
		$array = C('functionToModel');//require(APP_PATH.'Lib/ORG/FuncToModel.php');
	...
	public function commondetail(){
		...
		$className='FunctionLibrary_'.I('get.module');
		if (class_exists($className)){
			$classInstance=new $className($this->token,$sub);
			$o=$classInstance->index();
			/*
...
/* 		if (C('server_topdomain')=='pigcms.cn'){
	...
	public function Img(){
		...
		$db=M('Img');
		$ssimg = I('post.ssimg','htmlspecialchars,trim');
		if($ssimg){
	...
	public function GreetingCard(){
		...
		if ($list){
			foreach ($list as $item){
				array_push($items,array('id'=>$item['id'],'name'=>$item['title'],'linkcode'=>'{siteUrl}/index.php?g=Wap&m=Greetingcard&a=index&token='.$this->token.'&wecha_id={wechat_id}&id='.$item['id'],'linkurl'=>'','keyword'=>$item['keyword']));
			}
		}
		//
		$this->assign('list',$items);
