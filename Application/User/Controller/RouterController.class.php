...
class RouterController extends UserController{
	...
	public function lists(){
		...
		$page=new \Think\Page($pageinfo['total'],10);
		$this->display();
	}
	...
	public function set(){
		...
		if (IS_POST){
			if (isset($_GET['id'])){
				...
				//$this->curlPost($this->server.'api/node/delete?id='.trim(I('post.gw_id')).'&a='.substr(trim(C('router_key')),0,16).'&b='.substr($wxuser['routerid'],0,16));
				//
				$rt=$this->curlPost($this->server.'api/node/delete?id='.trim(I('post.gw_id')).'&a='.substr(trim(C('router_key')),0,16).'&b='.substr($wxuser['routerid'],0,16));
				$data=array(
				'gw_id'=>I('post.gw_id'),
				'gw_name'=>I('post.gw_name'),
				'wx_bind_type'=>1,
				'public_wechat_id'=>$wxuser['weixin'],
				'qrcode_url'=>I('post.qrcode_url'),
				'a'=>substr(trim(C('router_key')),0,16),
				'b'=>substr($wxuser['routerid'],0,16)
				);
				...
				if (intval($arr['ret'])==0){
					...
					'qrcode'=>I('post.qrcode_url'),
					'time'=>time(),
					'token'=>$this->token
					);
					$row['gw_id']=I('post.gw_id');
	...
	public function config(){
		...
		if (IS_POST){
			...
			'welcome_img'=>I('post.welcome_img'),
			'other_img'=>I('post.other_img'),
			'a'=>substr(trim(C('router_key')),0,16),
			...
			if ($record){
				...
				//
				M('Keyword')->where(array('token'=>$this->token,'module'=>'Router_config'))->save(array('keyword'=>I('post.keyword')));
				//
				...
				$id=$db->add($data1);
				//
				M('Keyword')->add(array('keyword'=>I('post.keyword'),'pid'=>$id,'token'=>$this->token,'module'=>'Router_config'));
				//
				if (intval($arr['ret'])==0){
