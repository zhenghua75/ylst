...
class UpyunController extends UserController{
	public $token;
	public $bucket;
	...
	function localUpload(){
		...
		if(!$info) {
			...
			$siteUrl = C('site_url');
				...
				$strPath=$upload->rootPath.$info[$key]['savepath'].$info[$key]['savename'];
				$strRealPath = realpath($strPath);
				$msg = $siteUrl.str_replace($_SERVER['DOCUMENT_ROOT'],'',$strRealPath);
				if (isset($_POST['width'])&&intval($_POST['width'])){
					$image = new \Think\Image(1,$strRealPath);
					$image = $image->thumb(intval($_POST['width']),intval($_POST['height']));
					$strImagePath = $upload->rootPath.$info[$key]['savepath'].'thumb_'.$info[$key]['savename'];
					
					$image->save($strImagePath);
					$strRealImagePath = realpath($strImagePath);
					$msg = $siteUrl.str_replace($_SERVER['DOCUMENT_ROOT'],'',$strRealImagePath);
				}
			M('Users')->where(array('id'=>$this->user['id']))->setInc('attachmentsize',intval($info[$key]['size']));
			M('Files')->add(array('token'=>$this->token,'size'=>intval($info[$key]['size']),'time'=>time(),'type'=>$info[$key]['ext'],'url'=>$msg));
		}
...
class UpYunException extends \Think\Exception {/*{{{*/
