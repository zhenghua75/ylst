...
class MembercardController extends UserController{
	...
	public function replyInfoSet(){
		...
		if (IS_POST){
			$memberArr=array();
			$nomemberArr=array();
			$memberArr['title']=I('post.title');
			$memberArr['info']=I('post.info');
			$memberArr['picurl']=I('post.picurl');
			$memberArr['token']=$this->token;
			$memberArr['apiurl']=I('post.apiurl');
			$memberArr['infotype']='membercard';
			
			$nomemberArr['title']=I('post.title1');
			$nomemberArr['info']=I('post.info1');
			$nomemberArr['picurl']=I('post.picurl1');
			$nomemberArr['token']=$this->token;
			$nomemberArr['apiurl']=I('post.apiurl');
	...
	public function create(){
		...
		if (IS_POST){//删除操作
			...
			$this->success('删除成功',U('Membercard/create',array('token'=>$this->token,'id'=>$this->thisCard['id'])));
		}else {
	...
	public function create_add(){ 
		...
		if (($end - $stat) > 100) {
			...
			if($back!=false){
				$this->success('恭喜您共开了'.$j.'张会员卡',U('Membercard/create',array('token'=>session('token'),'id'=>$this->thisCard['id'])));
			}else{
				$this->error('没开成任何会员卡');
			}
		}else{
	...
	public function add_gifts(){
		...
		if(IS_POST){
			if(D('Member_card_gifts')->create()){
					$_POST['cardid']= $cardid;
					$_POST['token'] = $this->token;
					$_POST['start'] = strtotime(I('post.start'));
					$_POST['end'] 	= strtotime(I('post.end'));
					if(D('Member_card_gifts')->add($_POST)){
						$this->success('添加成功',U('Membercard/gifts',array('token'=>$this->token,'id'=>$cardid)));
					}
			}else{
	...
	public function edit_gifts(){
		...
		if(IS_POST){
			if(D('Member_card_gifts')->create()){
				$_POST['start'] = strtotime(I('post.start'));
				$_POST['end'] 	= strtotime(I('post.end'));
				$where 	= array('token'=>$this->token,'cardid'=>$cardid,'id'=>I('post.gid','intval'));		
				D('Member_card_gifts')->where($where)->save($_POST);
				$this->success('修改成功',U('Membercard/gifts',array('token'=>$this->token,'id'=>$cardid)));
			}else{
				$this->error(D('Member_card_gifts')->getError());
			}
		}else{
	...
	public function del_gifts(){
		...
		if(M('Member_card_gifts')->where($where)->delete()){
			$this->success('删除成功',U('Membercard/gifts',array('token'=>$this->token,'id'=>$id)));
		}
	}
	
	public function get_value(){	
		$cardid 	= I('get.cardid',0,'intval');
		$item_attr 	= I('get.item_attr',0,'intval');
		$now 		= time();
		$result 	= array();
	...
	*/
	...
	function noticeSet(){
		...
		if (IS_POST){
			...
			$this->success('设置成功',U('Membercard/notice',array('token'=>session('token'),'id'=>$this->thisCard['id'])));
		}else {
			if (isset($_GET['noticeid'])){
	...
	function noticeDelete(){
		...
		$this->success('操作成功',U('Membercard/notice',array('token'=>session('token'),'id'=>$this->thisCard['id'])));
	}
	...
	public function privilege_add(){
		...
		if(IS_POST){
			...
			$this->success('操作成功',U('Membercard/privilege',array('token'=>session('token'),'id'=>$this->thisCard['id'])));
		}else{
			if (isset($_GET['itemid'])){
	...
	public function privilege_del(){
		...
		$member_card_vip=M('Member_card_vip');
		$data=$member_card_vip->where(array('token'=>$this->token,'id'=>I('get.itemid')))->delete();
		if($data==false){
			$this->error('服务器繁忙请稍后再试');
		}else{
			$this->success('操作成功',U('Membercard/privilege',array('id'=>$this->thisCard['id'],'token'=>$this->token,'type'=>4)));
		}
	}
	...
	public function coupons(){
		...
		$is_weixin 	= I('get.is_weixin',0,'intval');
	...
	public function coupons_set(){
		...
		if(IS_POST){
			...
			if ('0' == $type || '0' == $info['type']) {
				if ($least_cost <= 0) {
					$this->error('启用金额为正整数', U('Membercard/coupons'));
				} else {
					$_POST['least_cost'] = $least_cost;
				}
				if ($reduce_cost <= 0) {
					$this->error('减免金额为正整数', U('Membercard/coupons'));
				} else {
			...
			if(D('Member_card_coupon')->create()){
				...
				if($info){
					if($this->wxuser['wx_coupons'] && $info['is_weixin'] == 1){
						$this->error('非法修改，请重新操作',U('Membercard/coupons'));
					}
					
					if(D('Member_card_coupon')->where(array('token'=>$this->token,'id'=>I('post.id','intval')))->save()){
						$this->success('修改成功',U('Membercard/coupons',array('token'=>$this->token,'itemid'=>I('post.id','intval'))));
					}else{
						$this->error('修改失败');
					}
					...
					if($id){
						...
						if($coupon_type == 'weixin'){
							...
							if($res['errcode'] == 0){
								...
								$this->success('添加成功',U('Membercard/coupons'));
							}else{
								$err 	= '添加失败：'.$res['errmsg'].'';
	...
	public function coupons_save(){
		...
		if(IS_POST){
			...
			if($res['errcode'] == 0){
				$_POST['is_check']  = 0;
				M('Member_card_coupon')->where(array('token'=>$this->token,'id'=>I('post.id','intval')))->save($_POST);
				$this->success('修改成功',U('Membercard/coupons',array('token'=>$this->token,'itemid'=>I('post.id','intval'))));
			}else{
	...
	public function coupons_del(){
		...
		if($data==false){
			...
			$this->success('操作成功',U('Membercard/coupons',array('token'=>$this->token)));
		}
	}

	public function coupons_stock(){
		
		$itemid = I('post.data_id','intval');
	...
	public function coupon_edit(){
		...
		if(IS_POST){
			...
			$now=time();
			if (isset($_GET['itemid'])){
				$data=$member_card_coupon_db->where(array('token'=>session('token'),'id'=>I('get.itemid')))->find();
			}else {
				$data['statdate']=$now;
	...
	public function coupon_del(){
		$this->_isUseRecordExist(3,$_GET['itemid']);
			$data=M('Member_card_coupon')->where(array('token'=>session('token'),'id'=>I('get.itemid')))->delete();
			if($data==false){
				$this->error('没删除成功');
			}else{
				$this->success('操作成功',U('Membercard/coupon',array('id'=>$this->thisCard['id'],'token'=>$this->token,'type'=>2)));
	...
	public function integral_edit(){
		...
		if(IS_POST){
			...
			$now=time();
			if (isset($_GET['itemid'])){
				$data=$member_card_inergral_db->where(array('token'=>$this->token,'id'=>I('get.itemid')))->find();
			}else {
				$data['statdate']=$now;
				$data['enddate']=$now+10*24*3600;;
			}
	...
	public function integral_del(){
		$this->_isUseRecordExist(2,$_GET['itemid']);
			$data=M('Member_card_integral')->where(array('token'=>session('token'),'id'=>I('get.itemid')))->delete();
			if($data==false){
				$this->error('服务器繁忙请稍后再试');
			}else{
				$this->success('操作成功',U('Membercard/integral',array('id'=>$this->thisCard['id'],'token'=>$this->token,'type'=>3)));
	...
	public function staffSet(){
		...
		if(IS_POST){
			...
			$this->success('操作成功',U('Membercard/staff',array('token'=>session('token'),'id'=>$this->thisCard['id'])));
		}else{
			$company_db=M('Company');
			$companys=$company_db->where(array('token'=>$this->token))->order('id ASC')->select();
	...
	public function staffDelete(){
		...
		$data=$company_staff_db->where(array('token'=>session('token'),'id'=>I('get.itemid')))->delete();
		$this->success('操作成功',U('Membercard/staff',array('id'=>$this->thisCard['id'],'token'=>$this->token)));
		}
	}
		...
		if ($members){
			...
			foreach ($members as &$member) {
				if (isset($usersArr[$member['wecha_id']])) {
					...
					if ($thisUser && (empty($thisUser['truename']) || empty($thisUser['wechaname']) || empty($thisUser['tel']))) {
						if ($new = D('Userinfo')->updateInfo($thisUser['id'])) {
							$thisUser['truename'] = $new['truename'];
							$thisUser['wechaname'] = $new['wechaname'];
							$thisUser['tel'] = $new['tel'];
						}
					}
	...
	public function memberExpense(){
		...
		if (IS_POST){
			$arr=array();
			$arr['itemid']	= intval(I('post.itemid'));
			$arr['wecha_id']= I('post.wecha_id');
			$arr['expense']	= intval(I('post.money'));
			$arr['time']	= time();
			$arr['token']	= $this->token;
			$arr['cat']	 	= 3;
			$arr['staffid']	= intval(I('post.staffid'));
			$arr['usecount']= 0;
			$arr['cardid']	= I('get.id');
			$set_exchange = M('Member_card_exchange')->where(array('cardid'=>I('get.id')))->find();
...
//导出会员卡
	...
	public function exportCard(){
		...
		foreach ($result as $uid => $res) {
			...
			$objActSheet->setCellValue('A' . $index, $res['number']);
			if (empty($res['truename']) && isset($index_id['truename'])) {
				$res['truename'] = $attachs[$index_id['truename']];
			}
			$objActSheet->setCellValue('B' . $index, $res['truename']);
			
			if (empty($res['tel']) && isset($index_id['tel'])) {
				$res['tel'] = $attachs[$index_id['tel']];
			}
			$objActSheet->setCellValue('C' . $index, $res['tel'] . ' ');
			
			$objActSheet->setCellValue('D' . $index, $res['total_score']);
			...
			$objActSheet->setCellValue('E' . $index, $res['sex']);
			...
			$objActSheet->setCellValue('G' . $index, $res['bornmonth']);
			
			if (empty($res['bornday']) && isset($index_id['bornday'])) {
				$res['bornday'] = $attachs[$index_id['bornday']];
			}
			$objActSheet->setCellValue('H' . $index, $res['bornday']);
			
			if (empty($res['portrait']) && isset($index_id['portrait'])) {
				$res['portrait'] = $attachs[$index_id['portrait']];
			}
			$objActSheet->setCellValue('I' . $index, $res['portrait']);
			
			if (empty($res['qq']) && isset($index_id['qq'])) {
				$res['qq'] = $attachs[$index_id['qq']];
			}
			$objActSheet->setCellValue('J' . $index, $res['qq']);
			$res['getcardtime'] = $res['getcardtime'] ? date('Y-m-d', $res['getcardtime']) : '暂无';
			$objActSheet->setCellValue('K' . $index, $res['getcardtime']);
			$objActSheet->setCellValue('L' . $index, $res['expensetotal']);
			$objActSheet->setCellValue('M' . $index, $res['balance']);
			$objActSheet->setCellValue('N' . $index, $res['wecha_id']);
			
			foreach ($attachs as $fid => $fvalue) {
				if ($id_field[$fid]) continue;
				if (isset($key_id[$fid])) {
					$objActSheet->setCellValue($key_id[$fid] . $index, $fvalue . ' ');
				}
			}
			$index ++;
		}
...
//删除记录
	...
	public function donate_set(){
		...
		if(IS_POST){
			$min_price 		= I('post.min_price',0,'floatval');
			$max_price 		= I('post.max_price',0,'floatval');
			$donate_price 	= I('post.donate_price',0,'floatval');
			$is_open 		= I('post.is_open',0,'intval');
			$intro 			= I('post.donate_intro');
			...
			if($info){
				...
				$this->success('修改成功',U('Membercard/donate',array('token'=>$this->token,'id'=>$data['cardid'])));
			}else{
				if(M('Member_card_donate')->add($data)){
					$this->success('添加成功',U('Membercard/donate',array('token'=>$this->token,'id'=>$data['cardid'])));
				}
			}
	...
	public function donate_del(){
		...
		if(M('Member_card_donate')->where($where)->delete()){
			$this->success('删除成功',U('Membercard/donate',array('token'=>$this->token,'id'=>$cardid)));
		}
	}
	...
	public function test_check()
	{
		...
		$this->success('校验中', U('Membercard/test_check', array('p' => $page + 1, 'token' => $this->token)));
	}
}