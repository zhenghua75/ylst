...
class SeckillController extends UserController{
	...
	public function action_add()
	{
		...
		if (IS_POST) {
			...
			if ($_POST["id"] && !$_POST["id"]) {
				...
				$this->handleKeyword(I("post.id", "intval"), "Seckill", I("post.reply_keyword"));
				...
				if ($res = $action->add()) {
					$this->handleKeyword($res, "Seckill", I("post.reply_keyword"));
	...
	public function order_man()
	{
		...
		$Page = new \Think\Page($count[0]["cc"], 20);
		$show = $Page->show();
		$patterns = array();
		$patterns[0] = "/m=Seckill/";
		$replacements = array();
