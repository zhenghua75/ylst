...
class MessageController extends UserController{
	...
	public function uploadImage() {
		$html = I('post.html','', 'html_entity_decode');
	...
	public function index(){
		...
		if (IS_POST){
			$msgtype 	= I('post.send_type',0,'intval');
			$openid		= rtrim($_POST['openid'],',');
			$wechatgroupid = I('post.wechatgroupid',0,'intval');
			...
			if ($access_token){
				...
				if($str){
					...
					if($rt['rt']){
						...
						$row['status'] 	= 0;
						$row['send_type'] = I('post.send_type',0,'intval');
