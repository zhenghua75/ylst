...
class AppleGameController extends LotteryBaseController{
	public $lotteryType;
	...
	public function detail(){
		...
		$lid = (int)$_GET['id'];
		$Lottery = M('Lottery')->where(array('token'=>$this->token,'id'=>$lid))->find();
		//time 是得分
		...
		foreach($list as $k=>$v){
			...
			if($v['usenums'] > $Lottery['canrqnums']){
				$list[$k]['usenums'] = $Lottery['canrqnums'];
			}
		}
