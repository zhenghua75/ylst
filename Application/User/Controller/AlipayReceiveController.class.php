...
class AlipayReceiveController extends BaseController {
	public $user;
	...
	public function post(){
		...
		if(I('get.price')==false||I('get.uname')==false)$this->error('价格和用户名必须填写');
		...
		$seller_email =trim(C('alipay_name'));
		 //商户订单号
		$out_trade_no = I('get.uid').'_'.time();
		//商户网站订单系统中唯一订单号，必填
		//订单名称
		$subject ='充值vip'.I('get.group').'会员'.I('get.num').'个月';
		//必填
		//付款金额
		$total_fee =(int)I('get.price');
		...
		$body = $subject;
		$data=M('Indent')->data(			
		array('uid'=>intval($_GET['uid']),'month'=>intval(I('get.num')),'title'=>$subject,'uname'=>I('get.uname'),'gid'=>I('get.groupid'),'create_time'=>time(),'indent_id'=>$out_trade_no,'price'=>$total_fee))->add();
