...
class StoreController extends WapController{
	...
	public function _initialize() 
	{
		...
		if ($cid) {
			...
			session("session_company_{$this->token}", $this->mainCompany['id']);
		}
		...
		if ($twitter_set && empty($this->wecha_id) && empty($this->mytwid) && empty($login) && !in_array(ACTION_NAME, array('register', 'login'))) {
			...
			//点击链接时的推广记录
			session("login", 1);
			$this->redirect('Store/login', array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'twid' => $this->_twid, 'rget' => 1));
		}
...
// 			D('Userinfo')->where(array('id' => $this->fans['id']))->save(array('twid' => $twid));
		...
		$this->assign('staticFilePath', '/Public/wap/common/css/store');
...
// 		}
	...
	public function select()
	{
		...
		if (count($company) == 1) {
			$this->redirect('Store/cats',array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'cid' => $company[0]['id'], 'twid' => $this->_twid));
		}
	...
	{
		$this->redirect('Store/cats',array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'twid' => $this->_twid, 'cid' => $this->_cid));
	}
	...
	public function cats() 
	{
		...
		$catemenu[0] = array('id' => 0, 'name' => '所有商品', 'picurl' => '/Public/static/store/m-act-cat.png', 'k' => 0, 'vo' => array(), 'url' => U('Store/products', array('token'=> $this->token,'wecha_id'=> $this->wecha_id,'cid' => $this->_cid)));
		$catemenu[1] = array('id' => 1, 'name' => '购物车', 'picurl' => '/Public/static/store/m-act-cart.png', 'k' => 1, 'vo' => array(), 'url' => U('Store/cart', array('token'=> $this->token,'wecha_id'=> $this->wecha_id,'cid' => $this->_cid)));
		$catemenu[2] = array('id' => 2, 'name' => '查物流', 'picurl' => '/Public/static/store/m-act-wuliu.png', 'k' => 2, 'vo' => array(), 'url' => U('Store/my', array('token'=> $this->token,'wecha_id'=> $this->wecha_id,'cid' => $this->_cid)));
		$catemenu[3] = array('id' => 3, 'name' => '用户中心', 'picurl' => '/Public/static/store/user2.png', 'k' => 3, 'vo' => array(), 'url' => U('Store/myinfo', array('token'=> $this->token,'wecha_id'=> $this->wecha_id,'cid' => $this->_cid)));
		$this->assign('catemenu', $catemenu);
		$set = M("Product_setting")->where(array('token' => $this->token, 'cid' => $this->_cid))->find();
		if (isset(C('ClassifyTpls')[$set['tpid'] - 1]['tpltypename'])) {
			$t = C('ClassifyTpls')[$set['tpid'] - 1]['tpltypename'];
			
			$cateMenuFileName = "Index/menuStyle".$set['footerid'];
...
// 		$products = $this->product_model->where($where)->order("sort ASC, " . $order.' '.$method)->limit($start . ',' . $pageSize)->select();
	...
	public function product() 
	{
		...
		if (empty($product)) {
			$this->redirect('Store/products',array('token' => $this->token,'wecha_id' => $this->wecha_id, 'twid' => $this->_twid));
		}
...
// 		}
	...
	public function deleteCart()
	{
		...
		$_SESSION[$this->session_cart_name] = serialize($carts);
		$this->redirect('Store/cart',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'], 'twid' => $this->_twid));
	}
...
// 		$this->assign('alipayConfig', $alipayConfig);
		...
		if (empty($data[0])) {
			$this->redirect('Store/cart', array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'twid' => $this->_twid));
		}
		if (isset($data[1])) {
			foreach ($data[1] as $pid => $row) {
				$totalCount += $row['total'];
		...
		if($this->wxuser['winxintype'] ==3 && $this->wxuser['oauth'] == 1 && isWechat){
	...
	public function deleteOrder()
	{
		...
		$this->redirect('Store/my', array('token' => $_GET['token'], 'wecha_id' => $_GET['wecha_id'], 'twid' => $this->_twid));
	}
...
// 	   $orderid = $_GET['orderid'];
		...
		if(isset($_GET['nohandle'])){
			//执行跳转
			$this->redirect('Store/my',array('token' => $this->token,'wecha_id' => $this->wecha_id, 'twid' => $this->_twid));
		}else {
...
//                 $uid = $userInfo['id'];
	...
	{
        if (session('twid')) {
            $this->redirect('Store/myinfo', array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'twid' => $this->mytwid));
        }
...
// 		echo $this->mytwid;die;
	...
	public function logout()
	{
		...
		$this->redirect('Store/cats',array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'twid' => $this->_twid));
	}
