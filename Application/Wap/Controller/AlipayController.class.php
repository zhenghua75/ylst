...
class AlipayController extends BaseController
{
	...
	protected function _initialize()
	{
		...
		if (!$this->token) {
			...
			if (!$order) {
				$order = $product_cart_model->where(array("id" => intval(I("out_trade_no"))))->find();
			}
	...
	public function pay()
	{
		...
		$payHandel = new Api\payHandle($this->token, $_GET["from"]);
	...
	public function to_pay()
	{
		if (($_GET["pay_type"] != "Daofu") && ($_GET["pay_type"] != "Dianfu")) {
			$orderid = ($_GET["orderid"] ? $_GET["orderid"] : $_GET["single_orderid"]);
			$payHandel = new Api\payHandle($this->token, $_GET["from"]);
			$orderInfo = $payHandel->beforePay($orderid);
			$price = $orderInfo["old_price"];
			$coupon_id = ($_REQUEST["coupon_id"] ? intval($_REQUEST["coupon_id"]) : 0);
			...
			if ($coupon_id) {
				...
				if ($coupon["error"]) {
					...
					if ($price <= $coupon["reduce_cost"]) {
						$payHandel = new Api\payHandle($_GET["token"], $_GET["from"], "coupon");
		...
		if ($_GET["pay_type"] == "Weixin") {
			...
			header("Location:/index.php?g=Wap&m=Weixin&a=pay&price=" . $_GET["price"] . "&orderName=" . $_GET["orderName"] . "&single_orderid=" . $_GET["orderid"] . "&showwxpaytitle=1&from=" . $_GET["from"] . "&token=" . $_GET["token"] . "&wecha_id=" . $_GET["wecha_id"] . "&platform=" . intval($_GET["platform"]));
		}
		else {
