...
class IndexController extends WapController{
	...
	public function _initialize(){
		...
		if(array_key_exists('HTTP_REFERER', $_SERVER) && $_SERVER['HTTP_REFERER'] != "" && strpos($_SERVER['HTTP_REFERER'],'memberReg') === false && strpos($_SERVER['HTTP_REFERER'],'memberLogin') === false && strpos($_SERVER['HTTP_REFERER'],'PhoneVerify') === false && strpos($_SERVER['HTTP_REFERER'],'commonVerifyPhone') === false){
			session('R' , $_SERVER['HTTP_REFERER']);
		}
		$this->R = session('R');
	}
	...
	public function index(){
		...
		$this->assign('fun_list_new',$fun_list_new);
	      ...
	      $this->assign('currentTab',I('current'));
	      $this->display('099_index');
			...
			$tpldata['color_id']=intval($tpldata['color_id']);
				//获取模板信息
				$tplinfo=C('ClassifyTpls')[$tpldata['tpltypeid']];
	...
	public function lists(){
		...
		$tplinfo=C('ClassifyTpls')[$info['tpid']-1];
		$tpldata['tpltypeid'] = $tplinfo['tpltypeid'];
		$tpldata['tpltypename'] = $tplinfo['tpltypename'];
	...
	public function content($contid='',$cid=''){
		...
		//获取模板
		$tpldata=C('ContentTpls')[$classinfo['conttpid']];
