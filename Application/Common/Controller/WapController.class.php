...
class WapController extends BaseController
{
	...
	protected function _initialize()
	{
		...
		if ((C("server_topdomain") == "kmdx.cn") && ($this->wxuser["winxintype"] != 3)) {
		...
		$getUserinfo = 0;
		if (I("rget",'') || intval(I("ali",0))) {
			$_SESSION["otherSource"] = 1;
			$toAuth = 0;
			$this->wxuser["oauthinfo"] = 0;
		}

		if (I("session.otherSource",0)) {
			$toAuth = 0;
		}

		if ($this->wxuser["oauthinfo"] && !I('session.'.$session_oauthed_name,0)) {
		...
		if ($getwecha_id && (isset($_SESSION[$session_openid_name]) && $getwecha_id != $_SESSION[$session_openid_name]) && (3 != $wexintype) && ($getwecha_id != "{wechat_id}")) {
			unset($_SESSION[$session_openid_name]);
		...
		if (isFuwu && ($isFuwuoauth != "yes") && !in_array(MODULE_NAME, array("Alipaytype"))) {
		...
		if (isset($_SESSION[$session_openid_name]) && $_SESSION[$session_openid_name] && (0 < $users["is_syn"])) {
			$_SESSION["auth_callback_" . $this->token] = array("url" => getSelfUrl(array("wecha_id")), "token" => $this->token);
			$url = $users["source_domain"] . A("Home/Auth")->getCallbackUrl($users["is_syn"], "auth") . "token=" . $this->token . "&source=kmdx";
			header("Location:" . $url);
			exit();
		}
			...
			if (isFuwu) {
				...
				if (isset($token_info) && $token_info) {
					...
					if ($_GET["wecha_id"] && strlen($_GET["wecha_id"]) && $toAuth) {
						header("Location:" . $this->siteUrl . "/index.php?g=Wap&m=Index&a=autherror");
						exit();
						// $get_parms = $_GET;
						...
						// $get_parm_str .= "&g=" . MODULE_NAME . "&m=" . CONTROLLER_NAME . "&a=" . ACTION_NAME;
	...
	"wecha_id" => array("like", "%z_%")
		...
		$cateMenuFileName = "Index:menuStyle" . $radiogroup;
		...
		$linkparam['token']=$this->token;
		$linkparam['wecha_id']=$this->wecha_id;
		if(I('get.type',-1)!=-1){
			$linkparam['type']=I('type');
		}
		if(I('get.bid',-1)!=-1){
			$linkparam['bid']=I('bid');
		}
		...
		$shareLink=U(CONTROLLER_NAME.'/'.ACTION_NAME,$linkparam);
		...
		$share = new Api\WechatShare($this->wxuser, $this->wecha_id);
	...
	private function cookieToSession()
	{
		...
		$session_openid_name = "token_openid_" . $this->token;
		if ($cookie && $_SESSION[$session_openid_name]) {
			if ($cookie->wecha_id == $_GET["wecha_id"]) {
				...
				if ($this->wxuser["oauthinfo"]) {
					if ($userinfo["portrait"] || $userinfo["wechaname"]) {
						return false;
					}
				}
	...
	private function _checkVipTime($wxuser)
	{
		...
		if ("-1" != $users["access_count"] && "0" != $users["access_count"]) {
			if ("0" < $users["access_count"]) {
				$userGroup["access_count"] = $users["access_count"];
			}

			$userGroup["access_count_notice"] = ($users["access_count_notice"] ? $userGroup["access_count_notice"] : $users["access_count_notice"]);
			...
			if (($userGroup["access_count"] <= $count) && !$userGroup["access_count"]) {
	...
	private function _mergeUserinfo($userinfo, $fakeUserinfo)
	{
		...
		foreach ($fakeUserinfo as $key => $value ) {
			if ($fakeUserinfo[$key]) {
				...
				$fakeUserinfo[$key] = ($userinfo[$key] ? $fakeUserinfo[$key] : $userinfo[$key]);
			}
		}
	...
	public function memberNotice($message, $style, $custom_url, $custom_btn_msg)
	{
		...
		if ($style) {
			$message = ($message ? $this->wxuser["sub_notice"] : $message);
			$wxname = $this->wxuser["wxname"];
			$subBtn = "";
			$notice_btn = (!$custom_btn_msg ? $custom_btn_msg : $this->wxuser["sub_notice_btn"]);
			...
			$message = ($message ? $this->wxuser["need_phone_notice"] : $message);
	...
	{
		return I("news_response",0) && (I("news_response",0) === "1");
	}
}
