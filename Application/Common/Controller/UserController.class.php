...
class UserController extends BaseController
{
	...
	protected function _initialize()
	{
		...
		$trans = C('funcToModel');//include "./PigCms/Lib/ORG/FuncToModel.php";
		if (C("agent_version") && agentid) {
			$user_group_where = array("id" => session("gid"), "agentid" => agentid);
			$func_where = array("agentid" => agentid, "status" => 1);
		...
		if ((session("role_name") != "") && (session("role_name") == "staff")) {
			foreach ($group_func as $gk => $gv ) {
				$gvvv = ($trans[$gv] ? ucfirst($trans[$gv]) : ucfirst($gv));
				$group_func[$gk] = $gvvv;
			}
		...
		$is_syn = M("Wxuser")->where(array("token" => I("get.token")))->getField("is_syn");
		$is_syn = ($is_syn ? 0 : $is_syn);
		if (("Upyun" != MODULE_NAME) && ("Attachment" != MODULE_NAME)) {
			if ($_SESSION["role_name"] && (session("role_name") == "staff")) {
			...
			if ($_SESSION["administrator"] || $_SESSION["agentid"] || $_SESSION["uid"] || $_SESSION["wapupload"] || $_SESSION["userid"]) {
			}
		...
		$token = I('session.token');//$this->_session("token");
		...
		//if ((C("server_topdomain") != "pigcms.cn") && !C("close_user_notice")) {
			...
			//$updateUrl = $this->updateServerDomain . "oa/admin.php?m=notice&c=view&a=noticeByModule&group=" . MODULE_NAME . "&module=" . CONTROLLER_NAME . "&fileid=" . $updateRecord["version"];
	...
	protected function replaceUrl($url, $params)
	{
		$url = trim($url);
