...
class RBAC
{
	...
	static public function saveAccessList($authId)
	{
		if (NULL === $authId) {
	...
	static public function getRecordAccessList($authId, $module)
	{
		if (NULL === $authId) {
			$authId = $_SESSION[C("USER_AUTH_KEY")];
		}
	...
	static public function checkAccess()
	{
		if (C("USER_AUTH_ON")) {
			...
			if ((!$_module["no"] && !in_array(strtoupper(MODULE_NAME), $_module["no"])) || (!$_module["yes"] && in_array(strtoupper(MODULE_NAME), $_module["yes"]))) {
				...
				if ((!$_action["no"] && !in_array(strtoupper(ACTION_NAME), $_action["no"])) || (!$_action["yes"] && in_array(strtoupper(ACTION_NAME), $_action["yes"]))) {
					return true;
				}
				else {
					return false;
	...
	static public function AccessDecision($appName)
	{
		if (RBAC::checkAccess()) {
			...
			if ($_SESSION[C("ADMIN_AUTH_KEY")]) {
				...
				if (!$accessList[strtoupper($appName)][strtoupper($module)][strtoupper(ACTION_NAME)]) {
