...
class thirdApp
{
	...
	{
		return array("music", "yinle", "kuaidi", "tianqi", "baike", "suanming", "dianying");
		// return array("music", "yinle", "mengjian", "kuaidi", "tianqi", "baike", "geci", "suanming", "dianying", "feiji","huoche");
	}
	...
	// 	$url = apiServer::getApiUrltu() . "?key=" . apiServer::getApiKeyID()["key"] . "&info=" . $info . "&userid=".$this->uid;
	...
	public function dianying($name)
	{
		...
		$url = apiServer::getApiUrltu() . "?key=" . apiServer::getApiKeyID()["key"] . "&info=电影" . $name . "&userid=".$this->uid;
		$rt = $this->curlGet($url);
	...
	public function suanming($name)
	{
		...
		$url = apiServer::getApiUrltu() . "?key=" . apiServer::getApiKeyID()["key"] . "&info=算命" . $name . "&userid=".$this->uid;
		$rt = $this->curlGet($url);
		$data = json_decode($rt, true);

		if ($data["text"]) {
			$content=explode('<br>', $data["text"]);
			$strcontent=implode("\n", $content);
			return $strcontent;
		}
		else {
			$data = require_once EXTEND_PATH . "suanming.php";
			$num = mt_rand(0, 80);
			return $name . "\n" . trim($data[$num]);
		}
	}
	...
	// {
	// 	$name = implode("", $n);
	// 	$url = apiServer::getApiUrltu() . "?key=" . apiServer::getApiKeyID()["key"] . "&info=" . $name . "的歌词&userid=".$this->uid;
	// 	$rt = $this->curlGet($url);
	// 	$data = json_decode($rt, true);
	...
	public function baike($name)
	{
		...
		$url = apiServer::getApiUrltu() . "?key=" . apiServer::getApiKeyID()["key"] . "&info=百科" . $name."&userid=".$this->uid;
		$rt = $this->curlGet($url);
		$data = json_decode($rt, true);
	...
	public function kuaidi($param)
	{
		...
		$url = "http://m.kuaidi100.com/index_all.html?&postid=" . trim($param[0]);
		$link = "<a href='$url'>您好，您查询的快递单号为【" . trim($param[0]) . "】请点击查看详情</a>";
